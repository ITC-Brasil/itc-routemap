...
export function contarFiltrosAtivos(filtros: FiltrosHistorico): number {
  let n = 0
  if (filtros.periodo !== undefined) n++
  if (filtros.tecnico !== "todos") n++
  if (filtros.status !== "todos") n++
  return n
}
...
export function FiltrosHistoricoPopover({
  filtros,
  tecnicosDisponiveis,
  onChange,
}: Props) {
  ...
  const limparTudo = () => {
    onChange({ periodo: undefined, tecnico: "todos", status: "todos" })
  }
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="relative gap-2">
          ...
          Filtros
          {ativos > 0 && (
            <span className="flex h-4 w-4 items-center justify-center rounded-full bg-primary font-mono text-[10px] font-semibold text-primary-foreground">
              {ativos}
            </span>
          )}
        </Button>
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                  ...
                  onSelect={(range) => onChange({ ...filtros, periodo: range })}
                  locale={ptBR}
                  numberOfMonths={2}
                />
              </PopoverContent>
